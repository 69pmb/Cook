...
import pmb.cook.utils.FilesUtils;
import pmb.cook.utils.MarkdownUtils;
...
public class Cook {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Parent> folderParent = parents.stream().filter(p -> p.getFile().getIsDirectory()).collect(Collectors.toList());
    ...
    public static void buildIndexFile(List<Parent> parents) throws IOException {
        ...
        parents.stream().filter(Parent::isDirectory).forEach(parent -> {
            ...
            parent.getChildren().stream().filter(Child::isDirectory)
                    .forEach(child -> mark.buildLinkPage(parent.getName(), child.getName(), child.getGrandChildren().size()));
            List<Child> childrenNotDir = parent.getChildren().stream().filter(Predicate.not(Child::isDirectory)).collect(Collectors.toList());
            if (childrenNotDir.size() != parent.getChildren().size()) {
                mark.buildLinkPage(parent.getName(), "Divers", childrenNotDir.size());
            }
        });

        List<Parent> parentsNotDir = parents.stream().filter(Predicate.not(Parent::isDirectory)).collect(Collectors.toList());
    ...
    private static void buildPage(Parent parent, String previous, String next) throws IOException {
        ...
        parent.getChildren().stream().filter(Child::isDirectory).forEach(child -> {
            ...
            child.getGrandChildren().forEach(grandChild -> mark.buildLink(grandChild.getFile(), parent.getName(), child.getName()));
        });
