...
public class MarkdownUtils {
    ...
    public void buildLink(CookFile file, String parentFolder, String childFolder) {
        sb.append(new Link(file.getName(), MarkdownUtils.buildPath(parentFolder, childFolder, file.getFullName())))
    ...
    public void buildHeadind(String heading, Long count, int level) {
        sb.append(StringUtils.repeat("#", level) + " " + heading
                + Optional.ofNullable(count).map(c -> " (" + c + ")").orElse("")).append(NEW_LINE);
    }
    ...
    public void buildLinkPage(String parentName, String childName, int grandChildrenCount) {
        buildHeadind("- " + new Link(childName,
                parentName + FilesUtils.MD_EXT + "#"
                        + StringUtils.replace(StringUtils.lowerCase(childName).toString(), " ", "-") + "-"
                        + grandChildrenCount),
                Long.valueOf(grandChildrenCount), 3);
    }

    public static String buildPath(String parentFolder, String childFolder, String file) {
        Function<String, String> addSep = folder -> Optional.ofNullable(folder).map(dir -> folder + File.separator)
                .orElse("");
        return "." + File.separator + "docs" + File.separator + addSep.apply(parentFolder) + addSep.apply(childFolder)
        + Path.of(file);
    }
    ...
    public void buildArrowsHeader(String left, String right) {
        ...
        String leftArrow = Optional.ofNullable(left)
                .map(path -> MarkdownUtils.buildImgLink(array[0], "Page précedente", path + FilesUtils.HTML_EXT))
                .orElse("");
        String rightArrow = Optional.ofNullable(right)
                .map(path -> MarkdownUtils.buildImgLink(array[2], "Page suivante", path + FilesUtils.HTML_EXT))
                .orElse("");
