...
public class Cook {
    ...
    public static final Function<Path, String> GET_FILE_NAME = file -> WordUtils
            .capitalizeFully(StringUtils.substringBeforeLast(file.getFileName().toString(), "."));
    ...
    public static final Function<Path, String> GET_EXTENSIONS_SIZE = file -> " ("
            + FileUtils.byteCountToDisplaySize(file.toFile().length()) + " "
            + StringUtils.substringAfterLast(file.getFileName().toString(), ".") + ")";
            ...
            public static void main(String[] args) throws Exception {
                ...
                files.stream().filter(Files::isDirectory).forEach(file -> {
                    ...
                    .forEach(child -> sb.append(new Link(GET_FILE_NAME.apply(child), buildPath(file, child)))
                            .append(GET_EXTENSIONS_SIZE.apply(child)).append(NEW_LINE));
                });
                sb.append(new Heading((files.stream().filter(Files::isDirectory).count() + 1) + ". Divers", 1))
                .append(NEW_LINE);
                files.stream().filter(Predicate.not(Files::isDirectory)).sorted(FILE_NAME_COMPARATOR)
                .forEach(f -> sb.append(new Link(GET_FILE_NAME.apply(f), buildPath(null, f.getFileName())))
                        .append(GET_EXTENSIONS_SIZE.apply(f)).append(NEW_LINE));
